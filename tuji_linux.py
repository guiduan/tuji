...
def get_download_detail_page_url(detail_page_url):
    ...
    html = response.text
    pattern = re.compile(ur'免费下载地址:([\s\S]*?)\"><u>点此去下载')
...
def get_download_url(download_detail_page_url):
    ...
    html = response.text
    pattern = re.compile(
        ur'<big><a rel=\"external nofollow\" target=\"_blank" href=\"([\s\S]*?)\" download>下载点1')
    # pattern = re.compile(
...
def wget_file(download_url, local_path='./'):
    if not os.path.exists(local_path):
        os.makedirs(local_path)
    file_name = download_url.split('/')[-1]
...
if __name__ == '__main__':
    for page_id in range(1, 151):  # 第101页到第150页
